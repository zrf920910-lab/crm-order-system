  ...
  const handleSubmit = async () => {
    ...
    try {
      const r = await fetch('/api/auth', {
        ...
        body: JSON.stringify({ phone, password, action }),
      });
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg w-[380px]">
        ...
        <div className="flex mb-4 border rounded-lg overflow-hidden">
          <button onClick={() => { setAction('login'); setError(''); }}
            className={`flex-1 py-2 text-sm font-medium ${action === 'login' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>登录</button>
          <button onClick={() => { setAction('register'); setError(''); }}
            className={`flex-1 py-2 text-sm font-medium ${action === 'register' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600'}`}>注册</button>
        </div>
        ...
        <input type="password" placeholder="请输入密码（至少6位）" value={password}
        ...
        <div className="text-xs text-gray-400 text-center mt-4">
          {action === 'login' ? '首次使用请切换到注册' : '已有账号请切换到登录'}
        </div>
      </div>
    </div>
  );
