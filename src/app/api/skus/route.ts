...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (q) query = query.where(or(ilike(schema.skuPrices.skuName, `%${q}%`), ilike(schema.skuPrices.brand, `%${q}%`)));
    else if (letter) query = query.where(ilike(schema.skuPrices.skuName, `${letter}%`));
